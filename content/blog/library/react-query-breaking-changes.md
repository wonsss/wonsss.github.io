...
## 1. v4에서 breaking change: idle 상태가 제거됨

[useQuery return always isLoading with true when enabled is false #3975](https://github.com/TanStack/query/issues/3975)
...
- v3에서는 `idle` 상태가 있었기에, diabled인 쿼리들은 `idle` 상태부터 시작했습니다. `loading` 은 그 다음이었습니다.
...
- v4에서는 `idle` 상태가 사라지면서, diabled인 쿼리들이 `loading` 상태부터 시작하게 됩니다.
...
## 2. v5에서 breaking change: useQuery에서 onSuccess, onError, onSettled 콜백이 제거됨
...
- 왜 제거했을까?
  ...
  - 결론
    ...
    - 아래와 같은 멘탈 모델을 따르면 좋습니다.

## [참고] react-query 사용 시 멘탈 모델 - Service Layer Pattern

- useQuery에서 `onSuccess` 에 대한 여러 사용 케이스가 있지만, 다음 케이스들이 일반적입니다.
    1. UI 알림
        1. `useQuery` 의 경우, GET API 호출 성공 시 사용자에게 알림을 주는 경우가 있을 수 있으나, 그 유용성에 대해서는 의문입니다.
    2. Data 조작 (응답값을 조금 마사지하여 수동으로  `setQueryData` 호출)
        1. 데이터 조작은 순수한 네트워크 요청 단계에서 이뤄져야 합니다. 그게 아니면, 차라리 데이터 조작이 필요한 컴포넌트의 가까이에서 이뤄지게 하는 것이 적절합니다.
    3. 연쇄적 호출 (A 응답값이 B 요청을 하기 위해 필요한 경우)
        1. 연쇄적 호출은 `[dependent-queries](https://tanstack.com/query/v4/docs/react/guides/dependent-queries)` 패턴을 따라서 layer 4에서 하거나, layer 1에서 처리합니다.
