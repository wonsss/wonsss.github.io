...
- 2단계: 내비게이션(사이트 이동) 시작
  ...
  - `네트워크 스레드` 는 DNS Lookup 및 TLS 연결 설정과 같은 프로토콜을 거쳐 네트워크 요청(HTTP Request)을 처리한다.
    ...
    > TLS 연결 설정: 브라우저는 IP 주소에 있는 서버에게 랜덤한 숫자를 적은 번호표(Random Sequence)를 전달한다. 예를 들어, 서버는 이에 대해 1을 더한 번호표를 브라우저에게 응답한다. 브라우저도 다시 1을 더한 번호표를 브라우저게 응답한다. 이는 데이터를 주고 받기 위해, 서로 번호표를 주고 받는 3 Way-Handshake 과정이다. 3 Way-Handshake 과정이 끝나면 브라우저는 서버에게 해당 IP에 있는 데이터를 요청한다.
...
- 5단계: 내비게이션(사이트 이동) 실행
  ...
  - `브라우저 프로세스`는 `렌더러 프로세스`가 내비게이션을 실행했음을 확인하면, 새 사이트 정보를 반영하여 주소창, security indicator(자물쇠) 및 세션 히스토리 등을 업데이트한다.
