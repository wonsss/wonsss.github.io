...
- 삭제하는 site-available/default 의 기본 내용은 다음과 같으므로, 향후 참고하여 작성한다
...
### 7-11. Nginx 웹서버를 시작한다

```bash
sudo systemctl start nginx
```

### 7-12. 웹 서버 동작 상태를 확인할 수 있다
...
### 7-14. 만약 options-ssl-nginx.conf 파일이 없다면, 공식 레포에 있는 코드를 복사하여 입력한다
...
### 7-15. 안전한 키 길이를 가진 Diffie Hellman 알고리즘 키 생성
